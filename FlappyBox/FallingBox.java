...
public class FallingBox extends Box {
  ...
  public void move() {
    super.move();
    //setFallingSpeed(getFallingSpeed());
    // check if box is at bottom
    if (this.getY() <= GameConstant.BOX_LENGTH) {
      // set height to bottom to keep it still instead of falling through
      setY(GameConstant.BOX_LENGTH);
      //set falling velo and acceleration to 0 to prevent any movement
      setFallingSpeed(0);
